...
"""External multi-group packs for MiniMax H3 Motion Director (graph-wired inputs).

Two packer nodes produce ``MMX_MOTION_DIR_GROUPS`` lists that override the UI timeline
...
MMX_MOTION_DIR_GROUP = "MMX_MOTION_DIR_GROUP"
...
MMX_MOTION_DIR_GROUPS = MMX_MOTION_DIR_GROUP
...
def append_group(groups: list | None, group: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(group, dict) or group.get("kind") is None:
        raise ValueError("Invalid MMX_MOTION_DIR_GROUP payload.")
    out.append(group)
    return out
    ...
    if isinstance(raw, (list, tuple)):
        ...
        for item in raw:
            ...
            if isinstance(item, dict) and item.get("kind") is not None:
                ...
                raise ValueError(f"Invalid item in MMX_MOTION_DIR_GROUPS: {type(item)!r}")
        return out
    raise ValueError(f"Expected MMX_MOTION_DIR_GROUPS list, got {type(raw)!r}")
...
def validate_external_group_inputs(
    *,
    ...
    if i2v_linked and not i2v:
        raise ValueError("MiniMax H3 Motion Director: i2v_groups is connected but contains no groups.")
    if r2v_linked and not r2v:
        raise ValueError("MiniMax H3 Motion Director: r2v_groups is connected but contains no groups.")
    if i2v and r2v:
        raise ValueError(
            "MiniMax H3 Motion Director: connect either Image to Video groups (i2v_groups) "
    ...
    if i2v:
        if task_key not in I2V_FAMILY:
            raise ValueError(
                f"MiniMax H3 Motion Director: i2v_groups is connected but task_type is '{task_key}'. "
    ...
    if task_key != "r2v":
        raise ValueError(
            f"MiniMax H3 Motion Director: r2v_groups is connected but task_type is '{task_key}'. "
            "Set task to r2v."
        )
    for g in r2v:
        if g.get("kind") != "r2v":
...
def build_plan_from_external_groups(
    ...
    *,
    ...
    # Keep every group in the plan even during a selection run. Compacting the
