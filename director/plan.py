...
"""Parse MiniMax H3 Motion Director timeline JSON and prepare per-segment edit plans."""
...
def load_source_video_from_timeline(timeline: dict) -> torch.Tensor:
    ...
    if total <= 0:
        ...
        if not (video.get("frames") or []):
            raise ValueError("No frames in MiniMax H3 Motion Director timeline.")
...
def _parse_run_selection(timeline: dict, segment_count: int) -> frozenset[int] | None:
    ...
    if not indices:
        raise ValueError(
            "MiniMax H3 Motion Director: 「选择运行」已开启但未勾选任何片段/提示词组。请至少勾选一组再执行。"
        )
...
def build_director_plan(
    ...
    *,
    ...
    if not clips and not (load_timeline.get("video") or {}).get("frames"):
        raise ValueError(
            "No source video in MiniMax H3 Motion Director. Upload a video inside the node timeline UI before running."
        )
...
def plan_summary(plan: DirectorPlan) -> str:
    ...
    if mode in ("gen_blank", "gen_image", "prompt_batch", "image_batch", "fl2v"):
        ...
        lines = [
            f"MiniMax H3 Motion Director [{mode_label}] ({plan.edit_mode}): "
    ...
    lines = [
        f"MiniMax H3 Motion Director [{mode_label}] ({plan.edit_mode}): {plan.segment_count} segment(s), "
