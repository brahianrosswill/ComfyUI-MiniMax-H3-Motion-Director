...
"""Extract source audio aligned with MiniMax H3 Motion Director timeline (v2v / rv2v).

Independent of video tensors (does not touch decode / segment guidance). Same frame
selection as ``load_video_resampled``, with PCM-safe clocks:
  ...
  4. take exactly ``count / timeline_fps`` of audio (IMAGE / Combine clock).
     ...
     (that sounds like accelerated playback).
...
log = logging.getLogger("ComfyUI-MiniMax-H3-Motion-Director.audio")
...
def _probe_av_timing(path: str, *, fallback_fps: float) -> tuple[float, float, float]:
    """Return (video_pts0, audio_start, frame_dur) for PCM-to-picture sync.
...
def extract_timeline_audio(
    ...
    for path, pcm_start, out_dur, native0, file_fps in spans:
        ...
        # PTS media into a shorter window; that causes accelerated playback.
    ...
    if len(spans) > 50:
        log.warning(
            "Source audio: %d fragmented spans (timeline fps likely differs from source). "
