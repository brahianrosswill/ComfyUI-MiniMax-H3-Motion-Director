...
log = logging.getLogger("ComfyUI-MiniMax-H3-Motion-Director.video_io")
...
def _require_cv2():
    try:
        ...
        raise ImportError(
            "OpenCV is required for MiniMax H3 Motion Director video loading. "
...
def resolve_video_path(video: dict) -> str:
    ...
    if not video_file:
        raise ValueError("No video file in MiniMax H3 Motion Director timeline.")
...
def _decode_timeline_entries(
    ...
    *,
    ...
    clips = video_clips_from_timeline(timeline)
    if not clips:
        raise ValueError("No video clips in MiniMax H3 Motion Director timeline.")
